...
type InsightPanelProps = {
  ...
  actionStatus?: "idle" | "streaming" | "complete" | "error";
  variant?: "card" | "panel";
};
...
export function InsightPanel({
  ...
  const ActionIcon = actionSource === "ai" ? SparklesIcon : TargetIcon;
  const actionLabel = actionSource === "ai" ? "پیشنهاد هوشمند: " : "پیشنهاد: ";
  const actionContent = (
    <span aria-live="polite">
      {action || (actionStatus === "streaming" ? "در حال تحلیل داده‌ها…" : "")}
        ...
        <div className="flex gap-2">
          ...
          <div className="flex min-w-0 flex-col gap-2">
            ...
            <p className="rounded-lg bg-card/80 px-2.5 py-2 text-xs ring-1 ring-[var(--pulse-line)] sm:text-sm">
              <span className="font-medium text-[var(--pulse-violet)]">{actionLabel}</span>
              {actionContent}
            </p>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative mt-auto flex gap-2 rounded-lg border border-white/15 bg-white/10 p-2">
        <ActionIcon className="size-3.5 shrink-0 text-[var(--pulse-teal)]" aria-hidden="true" />
        <p className="text-xs leading-5 text-white/90 sm:text-sm sm:leading-6">
          <span className="font-semibold text-[var(--pulse-teal)]">{actionLabel}</span>
          {actionContent}
        </p>
      </div>
    </aside>
