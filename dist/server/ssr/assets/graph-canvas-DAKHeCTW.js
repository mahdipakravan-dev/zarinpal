import { c as __toESM, n as require_jsx_runtime, o as require_react, s as __commonJSMin } from "../index.js";
...
function q(n, u, t, i, r, o, e, f, c, a) {
	...
	n: if ("function" == typeof F) {
		h = e.length;
		try {
			if (x = u.props, M = F.prototype && F.prototype.render, I = (s = F.contextType) && i[s.__c], P = s ? I ? I.props.value : s.__ : i, t.__c ? k = (p = u.__c = t.__c).__ = p.__E : (M ? u.__c = p = new F(x, P) : (u.__c = p = new C(x, P), p.constructor = F, p.render = Q), I && I.sub(p), p.state || (p.state = {}), p.__n = i, v = p.__d = !0, p.__h = [], p._sb = []), M && null == p.__s && (p.__s = p.state), M && null != F.getDerivedStateFromProps && (p.__s == p.state && (p.__s = m$1({}, p.__s)), m$1(p.__s, F.getDerivedStateFromProps(x, p.__s))), y = p.props, d = p.state, p.__v = u, v) M && null == F.getDerivedStateFromProps && null != p.componentWillMount && p.componentWillMount(), M && null != p.componentDidMount && p.__h.push(p.componentDidMount);
			else {
...
//#endregion
//#region components/business-graph/graph-canvas.tsx
var import_jsx_runtime = require_jsx_runtime();
function getNodeId(endpoint) {
	return typeof endpoint === "string" ? endpoint : endpoint.id;
}
function GraphCanvasComponent({ graph, selectedNode, onSelectNode }) {
	const containerRef = (0, import_react.useRef)(null);
	const graphRef = (0, import_react.useRef)(void 0);
	...
	(0, import_react.useEffect)(() => {
		...
		const timer = window.setTimeout(() => graphRef.current?.zoomToFit(reduceMotion ? 0 : 500, 56), delay);
		return () => window.clearTimeout(timer);
	}, [graph, reduceMotion]);
	if (graph.nodes.length === 0) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", {
			...
			children: "داده کافی برای ساخت این گراف وجود ندارد"
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
			className: "m-0 max-w-md text-[11px] leading-7",
			children: "این پذیرنده در بازه دیتاست خرید Verified دارای payer_card_key ندارد. گراف مصنوعی نمایش داده نمی‌شود."
		})]
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ForceGraph2D, {
			ref: graphRef,
			graphData: graph,
			width: size.width,
			height: size.height,
			...
			nodeCanvasObject: (node, context, globalScale) => {
				...
				context.beginPath();
				context.arc(node.x ?? 0, node.y ?? 0, radius + (isSelected ? 2.5 : 0), 0, 2 * Math.PI);
				...
				context.stroke();
				if (isSelected || node.type === "category" || (node.val ?? 0) >= 10 || globalScale > 2.3) {
					context.font = `600 ${Math.max(3, 11 / globalScale)}px Vazirmatn, IRANSans, Tahoma, sans-serif`;
			...
			nodePointerAreaPaint: (node, color, context) => {
				context.beginPath();
				context.arc(node.x ?? 0, node.y ?? 0, Math.sqrt(node.val ?? 4) * 2.8, 0, 2 * Math.PI);
				context.fillStyle = color;
				context.fill();
			},
			linkColor: (link) => link.dashed ? "rgba(107,92,255,.36)" : "rgba(126,162,255,.28)",
			linkLineDash: (link) => link.dashed ? [3, 4] : null,
			linkWidth: (link) => Math.max(.4, Math.min(3.2, .4 + Math.log10(Number(link.weight) + 1) * .42)),
			linkLabel: (link) => {
				return `${getNodeId(link.source)} ← ${getNodeId(link.target)}${link.reasons?.length ? ` · ${link.reasons.join("، ")}` : ""}`;
			},
			linkDirectionalArrowLength: (link) => link.directed ? 4 : 0,
			linkDirectionalArrowRelPos: .8,
			linkDirectionalArrowColor: (link) => link.directed ? "rgba(126,162,255,.75)" : "rgba(0,0,0,0)",
			onNodeClick: (node) => onSelectNode(node),
			onBackgroundClick: () => onSelectNode(null),
			cooldownTicks: reduceMotion ? 0 : 100,
			onEngineStop: () => graphRef.current?.zoomToFit(reduceMotion ? 0 : 450, 80),
			d3VelocityDecay: .35,
			minZoom: .25,
			maxZoom: 8
		})
...
var GraphCanvas = (0, import_react.memo)(GraphCanvasComponent);
//#endregion
export { GraphCanvas };
